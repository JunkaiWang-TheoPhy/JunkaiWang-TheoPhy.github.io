    ...
    if (availableDates.length > 0) {
      ...
      container.innerHTML = `
        <div class="loading-container">
          ...
          <p>Please wait for the daily sync workflow to generate the first snapshot.</p>
        </div>
      `;
    }
  });
...
async function fetchGitHubStats() {
  try {
    const response = await fetch('https://api.github.com/repos/JunkaiWang-TheoPhy/JunkaiWang-TheoPhy.github.io');
